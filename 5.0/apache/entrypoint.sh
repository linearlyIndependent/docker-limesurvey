...
if [ "$LISTEN_PORT" != "80" ]; then
    ...
    sed -i "s/Listen 80\$/Listen $LISTEN_PORT/" /etc/apache2/ports.conf && sed -i "s/<VirtualHost \*:80>\$/<VirtualHost \*:$LISTEN_PORT>/" /etc/apache2/sites-available/000-default.conf
fi
...
cat /etc/apache2/apache2.conf /etc/apache2/ports.conf /etc/apache2/sites-available/000-default.conf /etc/apache2/sites-available/default-ssl.conf
